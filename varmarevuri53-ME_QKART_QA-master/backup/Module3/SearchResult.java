...
public class SearchResult {
    ...
    public String getTitleofResult() {
        ...
        WebElement titleSearch = parentElement.findElement(
    ...
    public Boolean openSizechart() {
        ...
        try {
            ...
            WebElement sizeChatButtonClick = parentElement.findElement(By.xpath(".//button[text()='Size chart']"));
            sizeChatButtonClick.click();
            status =true;
    ...
    public Boolean verifySizeChartExists() {
        ...
        try {
             ...
             */
            //Thread.sleep(1000);
             WebElement sizeChatButton = parentElement.findElement(By.xpath(".//button[text()='Size chart']"));
            if(sizeChatButton.isDisplayed()){
                String sizeChatButtonText =sizeChatButton.getText();
                if(sizeChatButtonText.equalsIgnoreCase("Size chart")){
                    status = true;
                }
            }
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
            ...
            for(int i=0; i<expectedTableHeaders.size();i++){
                ...
                WebElement actualTableHeaders= tableHeaders.get(i);
                String actualTableHeader = actualTableHeaders.getText();
            ...
            for(int i=0; i<expectedTableBody.size();i++){
                List<String> rowCount = expectedTableBody.get(i);
                for(int j=0; j<rowCount.size(); j++){
                    String expectedcolomnCount = rowCount.get(j);
                    int actualRowCount = i+1;
                    int actualColCount = j+1;
                    String actualContent = driver.findElement(By.xpath("//table//tbody//tr["+actualRowCount+"]/td["+actualColCount+"]")).getText();

                    if(!expectedcolomnCount.equals(actualContent)){
                        status =false;
